"""
    The main function is primarily responsible for training the model on the PEMS_BAY dataset.
"""
import copy
...
def Inverse_normalization(x, x_max, x_min):
    return x * (x_max - x_min) + x_min  # Inverse normalization
...
adj_mx, _ = load_adj("../Test_Data/" + data_name + "/" + "PEMS_BAY_adj.pkl", "doubletransition")
...
lr_rate = 0.002  # learning rate


input_len = 12  # Input length: H
num_id = 325  # Number of nodes: N
out_len = 12  # Output length: L
in_size = 1  # Input feature dimension: C
emb_size = 16  # embedding size C'
grap_size = 8  # graph embedding size D
layer_num = 2
dropout = 0.1
...
"""-----------------------------Model Saving & Test-----------------------------------------"""
# Model Save
model_save_path = "../IBN_Best_Model_Save/PEMS_BAY_Save/"
torch.save(best_net, model_save_path + "IBN_PEMS_BAY_7_0.25.pt")
with open(model_save_path + "IBN_PEMS_BAY_7_0.25.pt", "rb") as f:
    PEMS_BAY_Best_Model = torch.load(f)
PEMS_BAY_Best_Model.eval()
PEMS_BAY_Best_Model = PEMS_BAY_Best_Model.to(device2)
...
with torch.no_grad():
    ...
    for data in test_data:
        ...
        test_pre = PEMS_BAY_Best_Model(test_feature)
        if num == 0:
            all_pre = test_pre
