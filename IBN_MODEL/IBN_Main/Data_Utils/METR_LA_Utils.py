"""
    This section of code is primarily responsible for preprocessing the METR-LA dataset
...
data_file_path = folder_path + "metr-la" + ".h5"

with h5py.File(data_file_path, 'r') as f:
    # 'r' stands for read-only mode
    # The file contains only one key: ['df']
    # ['df'] contains four data components: ['axis0', 'axis1', 'block0_items', 'block0_values']
    group = f['df']
    raw_data_axis0 = group['axis0'][()]  # Contains the node IDs (variables), shape: [207, 1]
    raw_data_axis1 = group['axis1'][()]  # Time data stored as nanosecond Unix timestamps, shape: [34272, 1]
    raw_data_block0_items = group['block0_items'][()]  # Contains the variable node IDs [207, 1], identical to data_axis0; either can be used, but only one is needed.
    raw_data_block0_values = group['block0_values'][()]  # The data has shape [34272, 207], where each row represents a timestamp and each column represents a variable.

str_nodeID_data = [x.decode('utf-8') for x in raw_data_axis0]
raw_time_data = raw_data_axis1
feature_data = raw_data_block0_values
# Convert Unix timestamps to human-readable (normal) time
...
mask_id_1 = sorted(mask_id_1)   # Sort the variable ID
...
def feature_target(original_data, input_len, output_len):
    """
    :param original_data: The original raw data
    :param input_len: The length of the input sequence (in time steps) H
    :param output_len: The length of the output (prediction) sequence (in time steps)  L
    """
    fin_input = []
    fin_output = []
...
def Adjacency_Matrix_Generate(csv_path: str, node_name_list: list, node_num: int, normalized_k=0.1):
    """
    Args:
        csv_path (str): Path to the CSV file containing edge or distance data.
        node_name_list (list): List of node names to define the desired order of nodes,
            used to construct the adjacency matrix correctly (since original node order may be disordered).
        node_num (int): Number of nodes in the graph.
        normalized_k (float): Threshold for sparsifying the normalized distance matrix.
            Entries smaller than this value are set to 0.

    Returns:
        data_adj (np.ndarray): The constructed adjacency matrix.
    """
    ...
    for i in range(row_num):
        if str(distance_csv.iloc[i, 0]) not in sensor_id_to_ind or str(distance_csv.iloc[i, 1]) not in sensor_id_to_ind:
            continue
        data_adj[sensor_id_to_ind[str(distance_csv.iloc[i, 0])], sensor_id_to_ind[str(distance_csv.iloc[i, 1])]] = distance_csv.iloc[i, 2]
    ...
    # For the METR dataset, the adjacency matrix does not need to be diagonalized
    data_adj[data_adj < normalized_k] = 0

    return data_adj


test_adj = Adjacency_Matrix_Generate(folder_path+"distances_la_2012.csv", str_nodeID_data, n)
with open(folder_path+"METR_LA_adj.pkl", 'wb') as f:
