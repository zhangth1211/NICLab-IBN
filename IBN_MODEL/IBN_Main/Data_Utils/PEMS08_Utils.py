"""
    This section of code is primarily responsible for preprocessing the PEMS08 dataset
...
data_file_path = folder_path + 'PEMS08.npz'
...
def Adjacency_Matrix_Generate(csv_path: str, save_adj_path: str, node_num: int, normalized_k=0.1):
    """
    Args:
        csv_path (str): Path to the input CSV file containing edge data.
            Expected format: each row contains (from_node, to_node, cost/distance).
        save_adj_path (str): File path to save the generated adjacency matrix in .pkl format.
        node_num (int): Number of nodes in the graph (used to define the size of the adjacency matrix).
    ...
    Returns:
        data_adj (np.ndarray): The constructed adjacency matrix with shape [170, 170].
            Represents connectivity weights between nodes.
    """
...
PEMS08_adj = Adjacency_Matrix_Generate(distance_path, predefined_pkl_path, n)
print("The adjacency matrix has dimensions {0}".format(PEMS08_adj.shape))
print(PEMS08_adj)
